...
std::vector< glm::vec4> cameraKeyFrames;
std::vector< glm::vec4> cameraPath;
...
void printToScreen();
//produce un percorso liscio della camera
void interpolateCameraKeyFrames();
//ordina gli oggetti secondo la distanza dalla camera
vector<Object> sortByCameraDistance(vector<Object> obj);
...
void init_reflective_sphere() {
	...
	obj.material = MaterialType::NO_MATERIAL;
	obj.shading = ShadingType::REFLECTION;// PASS_THROUGH; //TEXTURE_ONLY; //  TODO REFLECTION;
...
void init_torus() {
	...
	obj.shading = ShadingType::TEXTURE_PHONG; //TEXTURE_ONLY;  // PASS_THROUGH; // TEXTURE_PHONG;  
	obj.name = "Torus";
	...
	for (i = 0; i < 64; i++) {
		for (j = 0; j < 64; j++) {
			double x = (double)j / ((double)64);
			double y = (double)i / ((double)64);
...
void drawScene() {
	...
	transparentObjs=sortByCameraDistance(transparentObjs);
	glDepthMask(GL_FALSE);
...
void refresh_monitor(int millis)
{
	if (isCameraMoving && cameraPath.size()>0) {
		currentIndexInPath++;
		if (currentIndexInPath >= cameraPath.size()) {
			currentIndexInPath = 0;
		}
		ViewSetup.position = cameraPath.at(currentIndexInPath);
	}
	glutPostRedisplay();
...
void keyboardDown(unsigned char key, int x, int y)
{
	...
	case 'n':
		...
		break;
	case ' '://spacebar inizia il movimento della camera o lo ferma
		isCameraMoving = !isCameraMoving;
		break;
	case 'k'://inserisce un keyframe per il movimento della camera
		cameraKeyFrames.push_back(ViewSetup.position);
		if(cameraKeyFrames.size()>2) interpolateCameraKeyFrames();
		break;
	case 'K'://rimuove l'ultimo keyframe inserito
		if(cameraKeyFrames.size()>0)cameraKeyFrames.pop_back();
		if (cameraKeyFrames.size() > 2) interpolateCameraKeyFrames();
		break;
	case 'd'://cancella tutti i keyframe, stoppa il movimento della camera e cancella il suo cammino
		isCameraMoving = false;
		cameraKeyFrames.erase(cameraKeyFrames.begin(), cameraKeyFrames.end());
		cameraPath.erase(cameraPath.begin(), cameraPath.end());
		break;
	default:
		break;
	}
...
void modifyModelMatrix(glm::vec3 translation_vector, glm::vec3 rotation_vector, GLfloat angle, GLfloat scale_factor)
{
	...
	case OCS: {
		glm::vec3 scalevec = glm::vec3(1.0, 1.0, 1.0) - rotation_vector + (rotation_vector * scale_factor);
		glm::mat4 trans = glm::mat4(1.0f);
		trans = glm::translate(trans, translation_vector);
		trans = glm::scale(trans, scalevec);
		trans = glm::rotate(trans, angle, rotation_vector);
		objects[selected_obj].M = objects[selected_obj].M * trans;
		break;
	}
	...
	case WCS: {
		...
		trans = glm::scale(trans, scalevec);
		trans = glm::rotate(trans, angle, rotation_vector);
		objects[selected_obj].M = inverse(inverse(objects[selected_obj].M) * trans);
		break;
	}
	default: break;
	}
...
void interpolateCameraKeyFrames() {
	...
	for (int k = 0; k <= ncurvepoints; k++) {
		//deCasteljau
		float t = (GLfloat)k / ncurvepoints;
		int dim = cameraKeyFrames.size();
		std::vector<glm::vec4> tempVector;
		for (int i = 0; i < dim; i++) {
			tempVector.push_back(cameraKeyFrames.at(i));
		}
		tempVector.push_back(cameraKeyFrames.at(0));//per tornare al punto di partenza

		for (int i = 1; i < dim+1; i++) {
			for (int j = 0; j < dim; j++) {
				tempVector.at(j).x = (1 - t) * tempVector.at(j).x + t * tempVector.at(j+1).x;
				tempVector.at(j).y = (1 - t) * tempVector.at(j).y + t * tempVector.at(j + 1).y;
				tempVector.at(j).z = (1 - t) * tempVector.at(j).z + t * tempVector.at(j + 1).z;
				tempVector.at(j).w = 0;
			}
		}
		cameraPath.push_back(tempVector.at(0));
	}
}
...
vector<Object> sortByCameraDistance(vector<Object> obj) {
	...
	while (!obj.empty()) {
		for (int i = 0; i < obj.size(); i++) {
			float distance = abs(glm::length(obj[i].M * glm::vec4(0.f, 0.f, 0.f, 1.f) - ViewSetup.position));
		...
		res.push_back(obj.at(maxindex));
		obj.erase(obj.begin()+maxindex);
		maxdistance = -1.0f;
		maxindex = -1;
	}
	return res;
